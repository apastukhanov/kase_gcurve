...
def update_trades_table1(data):
    ...
    # print('КАСЕ',curve)
    # returof"NelsonSiegelCurve({curve.beta0=:.4f}, {curve.beta1=:.4f}. {curve.beta2=:.4f}, {curve.tau=:.4f})"
...
def update_trades_table2(data):
    ...
    curve, status = calibrate_ns_ols(t, y, tau0=INL_TAU)
    # print('TN',curve)
    # return f"NelsonSiegelCurve({curve.beta0=:.4f}, {curve.beta1=:.4f}. {curve.beta2=:.4f}, {curve.tau=:.4f})"
    return pd.DataFrame([{'tradedate':df['tradedate'].iloc[0],'B0':curve.beta0/100, 'B1':curve.beta1/100, 'B2':curve.beta2/100, 'TAU':curve.tau}]).to_dict('records')
